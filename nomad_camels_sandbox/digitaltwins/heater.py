...
class heater:
    ...
    def _calc_convection_heatpower(self, temperature: float):
        return (self.heat_transfer_coefficient * self.area *
                (self.temperature_environment - temperature))
    
    
    def _calc_radiation_heatpower(self, temperature: float):
        return (self.emissivity * constants.sigma * self.area *
                (self.temperature_environment**4 - temperature**4))
    

    def _calc_total_heatpower(self, ):
        return (self.tec.heatpower(self.current, 
                                   self.temperature,
                                   self.temperature_environment) +
    ...
    def _calc_temperature_change(self):
        ...
        f = lambda temp, t: ((self.tec.heatpower(self.current, 
                                                 temp, Te) +
                              self._calc_convection_heatpower(temp) + 
                              self._calc_radiation_heatpower(temp)) / c / m)
